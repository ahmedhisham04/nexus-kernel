...
def page_garch():
    ...
    <div class="eq-block">
      ...
      <p>rₜ = μ + εₜ &nbsp;|&nbsp; εₜ = σₜ·zₜ, &nbsp; zₜ ~ {dist.upper()}</p>
              <p class="eq-cyan">σ²ₜ = ω + α₁·ε²ₜ₋₁ + β₁·σ²ₜ₋₁ + γ·(Leverage/Threshold)</p>
            </div>""", unsafe_allow_html=True)
            ...
            for nm, val, se, tv, pv in zip(res.params.index, res.params, res.std_err, res.tvalues, res.pvalues):
                rows_html += coef_row_html(nm, val, se, tv, pv)
            st.markdown(coef_table_html(rows_html), unsafe_allow_html=True)

            # ── Volatility Plot ──
            cond_vol = res.conditional_volatility
            fig = make_subplots(rows=2, cols=1, subplot_titles=[f"Returns: {ret_var}", "Conditional Volatility (σₜ)"], vertical_spacing=0.1)
            fig.add_trace(go.Scatter(x=s.index, y=s.values, line=dict(color=C["navy"], width=1)), row=1, col=1)
            fig.add_trace(go.Scatter(x=s.index, y=cond_vol, line=dict(color=C["cyan"], width=2), fill='tozeroy', fillcolor="rgba(56,189,248,0.15)"), row=2, col=1)
            fig = navy_fig(fig, 500)
            st.plotly_chart(fig, use_container_width=True)
            ...
            ai_text = f"""<p>The <strong>{mdl_type}</strong> model reveals {vol_type} volatility clustering.
            The persistence parameter (α+β) is {persist:.4f}.
            {'The half-life of a volatility shock is approx ' + f'{half_life:.1f} periods.' if persist < 1 else 'The process is explosive (IGARCH).'}
            Leverage effects (γ) measure asymmetric responses to negative vs. positive shocks.</p>"""
            st.markdown(f'<div class="ai-box"><h4>⬡ AI INTERPRETATION · VOLATILITY</h4>{ai_text}</div>', unsafe_allow_html=True)
